...
SDL_Window* gpu::SDLManager::getWindow() {
	return window;
};

SDL_Renderer* gpu::SDLManager::getRenderer() {
	return renderer.get();
};

std::shared_ptr<SDL_Renderer> gpu::SDLManager::getRendererShared() {
	return renderer;
};

AbsoluteDimensions gpu::SDLManager::getScreenDimensions() {
	int w, h;
