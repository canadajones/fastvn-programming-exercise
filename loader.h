...
class JSONLoader {
	...
	public:
	JSONLoader(std::string indexPath) : paths{loadIndex(indexPath)} {
		if (paths.size() < 1) {
	...
	Chapter loadChapter(uint w, uint h) {
		...
		// TODO: check the error code on this / make it throw an exception
		// I have no idea what happens if the JSON fed into this is malformed
